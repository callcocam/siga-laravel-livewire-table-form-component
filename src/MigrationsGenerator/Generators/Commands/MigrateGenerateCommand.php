...
namespace Call\MigrationsGenerator\Generators\Commands;

use Symfony\Component\Console\Input\InputOption;
use Symfony\Component\Console\Input\InputArgument;

use Call\MigrationsGenerator\Generators\Generator;
...
use Call\MigrationsGenerator\Generators\SchemaGenerator;
use Call\MigrationsGenerator\Generators\Syntax\AddToTable;
use Call\MigrationsGenerator\Generators\Syntax\DroppedTable;
use Call\MigrationsGenerator\Generators\Syntax\AddForeignKeysToTable;
use Call\MigrationsGenerator\Generators\Syntax\RemoveForeignKeysFromTable;
...
class MigrateGenerateCommand extends GeneratorCommand
{
    ...
    /**
     * @var \Call\MigrationsGenerator\Generators\SchemaGenerator
     */
    ...
    /**
     * @param \Call\MigrationsGenerator\Generators\Generator  $generator
     * @param \Call\MigrationsGenerator\Generators\Filesystem\Filesystem  $file
     * @param \Call\MigrationsGenerator\Generators\Compilers\TemplateCompiler  $compiler
    ...
    protected function generateTablesAndIndices(array $tables)
    {
        ...
        foreach ($tables as $table) {
            ...
            if (isset($this->fields['company_id'])) {
                unset($this->fields['company_id']);
            }
            if (isset($this->fields['companys_id'])) {
                unset($this->fields['companys_id']);
            }
    ...
    protected function getTemplateData()
    {
        ...
        return [
            'CLASS' => ucwords(\Illuminate\Support\Str::camel($this->migrationName)),
            'UP'    => $up,
            'DOWN'  => $down
        ];
    }
    ...
    {
        return call_migration_generate_path($this->getPathByOptionOrConfig('templatePath', 'migration_template_path'));
    }
    ...
    {
        $excludes = ['migrations','phinxlog','password_resets','failed_jobs','users','tenants','companies','address','contacts','documents','socials','roles','permissions','files','sliders','translations','makes'];
