...
class Iseed
{
    ...
    {
        return base_path() . config('generators.path');
    }
    ...
    public function repackSeedData($data)
    {
        ...
        if (!empty($data)) {
            foreach ($data as $row) {
                ...
                if (!is_string($row->id)) {
                    $rowArray['status'] = ['draft', 'published'][$row->status];
                }
                $dataArray[] = $rowArray;
            }
        }
    ...
    {
        $chunkSize = $chunkSize ?: config('generators.chunk_size');
    ...
    {
        $databaseSeederPath = base_path() . config('generators.path') . '/DatabaseSeeder.php';
